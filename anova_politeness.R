...
#
# Analysis of variance (ANOVA) code
# adapted from 
...
# Load data
##############################
setwd("~/Box/machine_learning_workup/ANOVA")
setwd("~/Box\ Sync/machine_learning_workup/ANOVA")
df_monocyte_genes_IGP <- read.csv('politeness_data.csv', 
                                  sep = ',', header = TRUE, 
                                  stringsAsFactors=FALSE, na.strings="..") # ,strip.white = TRUE)
# quote="" # for dealing with ' etc in files

summary(df_monocyte_genes_IGP)
colnames(df_monocyte_genes_IGP)
...
# conver to factor
df_monocyte_genes_IGP$gender <- factor(df_monocyte_genes_IGP$gender)
boxplot(scenario ~ frequency, data = df_monocyte_genes_IGP)
...
# http://www.bodowinter.com./tutorial/bw_LME_tutorial1.pdf
############################################################
xmdl = lm(frequency ~ gender, data = df_monocyte_genes_IGP, na.action = na.exclude) # na.omit
summary(xmdl)

##############################
# One-way ANOVA
##############################
oneway.test(formula = scenario ~ frequency, data = df_monocyte_genes_IGP)

##############################
# ANOVA
##############################
aov_out = aov(formula = scenario ~ frequency, data = df_monocyte_genes_IGP)
summary(aov_out)
...
contrasts(df_monocyte_genes_IGP$frequency)
...
# Dunnett (optimised for comparing all treatments to a single control); Games-Howell (doesnt assume equal variances); a
